...
always @(*) begin
    case (state)
        ...
        S_READ_RESERVED_EVAL: nstate = (addr_counter_q == 10'd1023) ? S_READ_OFMAP_REQ : S_READ_RESERVED_REQ;
...
always @(posedge clk or negedge rstn) begin
    if (!rstn) begin
        ...
        case (display_mode)
            2'd0: display_value_q <= score_sw_sync1 ? score_display_bcd : count_display_bcd;
            2'd1: display_value_q <= total_cycle_count[15:0];
            2'd2: display_value_q <= total_cycle_count[31:16];
            2'd3: display_value_q <= {4'h0, test_id, 3'd0, state};
            default: display_value_q <= 16'h0000;
        endcase
        scan_sel_q <= scan_sel;
