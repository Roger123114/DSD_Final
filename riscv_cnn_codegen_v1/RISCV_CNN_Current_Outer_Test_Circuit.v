...
reg [31:0] mem12_before_exec;
reg [31:0] mem13_before_exec;
...
begin
    ...
    test_read_word(10'd12, mem12_before_exec);
    test_read_word(10'd13, mem13_before_exec);

    if (mem12_before_exec[0] !== 1'b1) begin
        $display("[TB][ERROR] mem[12][0] was not high before execution. mem12=%h",
                 mem12_before_exec);
        error_count = error_count + 1;
        case_error_count = case_error_count + 1;
    end

    if (mem13_before_exec[0] !== 1'b0) begin
        $display("[TB][ERROR] mem[13][0] was high before execution. mem13=%h",
                 mem13_before_exec);
        error_count = error_count + 1;
        case_error_count = case_error_count + 1;
    end
...
begin
    ...
    feature_size = mem12_before_exec[6:1];

    if (feature_size != expected_feature_size) begin
        $display("[TB][ERROR] feature size mismatch: expected=%0d got=%0d pre_mem12=%h post_mem12=%h",
                 expected_feature_size, feature_size, mem12_before_exec, read_data_tmp);
        error_count = error_count + 1;
        case_error_count = case_error_count + 1;
    end
    ...
    $display("[TB] pre_mem12=%h post_mem12=%h mem13=%h feature_size=%0d output_start=%0d output_words=%0d",
             mem12_before_exec, `DUT_MEM_PATH[12], read_data_tmp, feature_size, output_start, output_words);
end
endtask
